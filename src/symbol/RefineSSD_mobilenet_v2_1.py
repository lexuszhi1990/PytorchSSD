...
def conv_bn(inp, oup, kernel_size=3, stride=1, padding=1, bias=False):
    return nn.Sequential(
        nn.Conv2d(inp, oup, kernel_size=kernel_size, stride=stride, padding=padding, bias=bias),
        nn.BatchNorm2d(oup),
        nn.ReLU6(inplace=True)
    )
...
def build_mobile_net_v2(width_mult=1., data_dim=3):
    ...
    # building first layer
    input_channel = int(interverted_residual_setting[0][1] * width_mult)
    # features.append(conv_bn(data_dim, input_channel))
    features.append(conv_bn(data_dim, input_channel, kernel_size=3, stride=2, padding=1))
...
class RefineSSDMobileNetV1(nn.Module):
    '''
        only support 16:9 shape images
    '''
    def __init__(self, num_classes=81, width_mult=1., base_channel_num=128, base_anchor_num=3, use_refine=True):
        super(RefineSSDMobileNetV1, self).__init__()
        self.num_classes = num_classes
        self.base_channel_num = base_channel_num
        self.base_anchor_num = base_anchor_num
        self.use_refine = use_refine
        self.base_channel_list = [ int(width_mult*num) for num in [32, 96, 320, 360, 420] ]

        self.base = nn.ModuleList(build_mobile_net_v2(width_mult=width_mult))
        self.appended_layer = nn.ModuleList([
                nn.Sequential(
                    nn.Conv2d(self.base_channel_list[2], self.base_channel_list[3], kernel_size=3, stride=1, padding=1),
                    nn.ReLU6(inplace=True),
                    nn.Conv2d(self.base_channel_list[3], self.base_channel_list[3], kernel_size=[2, 3], stride=2, padding=(0, 1)),
                    nn.ReLU6(inplace=True),
                ),
                nn.Sequential(
                    nn.Conv2d(self.base_channel_list[3], self.base_channel_list[4], kernel_size=3, stride=1, padding=1),
                    nn.ReLU6(inplace=True),
                    nn.Conv2d(self.base_channel_list[4], self.base_channel_list[4], kernel_size=[2, 3], stride=2, padding=(0, 1)),
                    nn.ReLU6(inplace=True),
                ),
            ])
        if self.use_refine:
            self.arm_loc = nn.ModuleList([
                    nn.Conv2d(self.base_channel_list[0], 4*self.base_anchor_num, kernel_size=3, stride=1, padding=1),
                    nn.Conv2d(self.base_channel_list[1], 4*self.base_anchor_num, kernel_size=3, stride=1, padding=1),
                    nn.Conv2d(self.base_channel_list[2], 4*self.base_anchor_num, kernel_size=3, stride=1, padding=1),
                    nn.Conv2d(self.base_channel_list[3], 4*self.base_anchor_num, kernel_size=3, stride=1, padding=1),
                ])
            self.arm_conf = nn.ModuleList([
                    nn.Conv2d(self.base_channel_list[0], 2*self.base_anchor_num, kernel_size=3, stride=1, padding=1),
                    nn.Conv2d(self.base_channel_list[1], 2*self.base_anchor_num, kernel_size=3, stride=1, padding=1),
                    nn.Conv2d(self.base_channel_list[2], 2*self.base_anchor_num, kernel_size=3, stride=1, padding=1),
                    nn.Conv2d(self.base_channel_list[3], 2*self.base_anchor_num, kernel_size=3, stride=1, padding=1),
                ])
        self.odm_loc = nn.ModuleList([
                nn.Conv2d(self.base_channel_num, 4*self.base_anchor_num, kernel_size=3, stride=1, padding=1),
                nn.Conv2d(self.base_channel_num, 4*self.base_anchor_num, kernel_size=3, stride=1, padding=1),
                nn.Conv2d(self.base_channel_num, 4*self.base_anchor_num, kernel_size=3, stride=1, padding=1),
                nn.Conv2d(self.base_channel_num, 4*self.base_anchor_num, kernel_size=3, stride=1, padding=1),
            ])
        self.odm_conf = nn.ModuleList([
                nn.Conv2d(self.base_channel_num, self.num_classes*self.base_anchor_num, kernel_size=3, stride=1, padding=1),
                nn.Conv2d(self.base_channel_num, self.num_classes*self.base_anchor_num, kernel_size=3, stride=1, padding=1),
                nn.Conv2d(self.base_channel_num, self.num_classes*self.base_anchor_num, kernel_size=3, stride=1, padding=1),
                nn.Conv2d(self.base_channel_num, self.num_classes*self.base_anchor_num, kernel_size=3, stride=1, padding=1),
            ])
        ...
        self.up_layers = nn.ModuleList([
                nn.ConvTranspose2d(self.base_channel_list[4], self.base_channel_num, kernel_size=[2, 3], stride=2, padding=(0, 1)),
                nn.ConvTranspose2d(self.base_channel_num, self.base_channel_num, kernel_size=[2, 3], stride=2, padding=(0, 1)),
                nn.ConvTranspose2d(self.base_channel_num, self.base_channel_num, kernel_size=2, stride=2, padding=0),
                nn.ConvTranspose2d(self.base_channel_num, self.base_channel_num, kernel_size=2, stride=2, padding=0),
            ])
        self.latent_layrs = nn.ModuleList([
    ...
    def forward(self, x, inference=False):
        ...
        # 1, 2, 4, 7, 11, 14, 17,
        base_output, arm_sources = self.base_forward(x, [6, 13, 17])
